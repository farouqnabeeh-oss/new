...
import { Truck, Info, Sparkles, Save } from "lucide-react";
...
export default function DeliveryDiscountsManager() {
    ...
    const [rules, setRules] = useState<Rule[]>([
        { id: "tier1", range: isAr ? "0 — 50 ₪" : "0 — 50 ₪", type: "none", value: 0 },
        { id: "tier2", range: isAr ? "50 — 100 ₪" : "50 — 100 ₪", type: "fixed", value: 5 },
        { id: "tier3", range: isAr ? "100 — 250 ₪" : "100 — 250 ₪", type: "fixed", value: 10 },
        { id: "tier4", range: isAr ? "250+ ₪" : "250+ ₪", type: "free", value: 0 },
    ]);
            ...
            try {
                ...
                if (data.rules && data.rules.length > 0) {
                    const mapped = data.rules.map((r: any, i: number) => ({
                        id: `tier${i + 1}`,
                        range: ["0 — 50 ₪", "50 — 100 ₪", "100 — 250 ₪", "250+ ₪"][i],
        ...
        <form onSubmit={handleSave} className="admin-profile-container flex flex-col gap-6" dir={isAr ? "rtl" : "ltr"}>
            ...
            <div className="flex items-center gap-4 bg-white p-5 sm:p-7 rounded-[30px] border border-gray-100 shadow-sm">
                ...
                <div>
                    <h2 className="text-lg sm:text-xl font-[900] text-gray-800 m-0 leading-tight">
                        {isAr ? "خصومات التوصيل الذكية" : "Smart Delivery Discounts"}
                    </h2>
                    <p className="text-[12px] text-gray-500 font-semibold m-0 mt-1">
                        {isAr
                            ? "خصم على سعر التوصيل حسب قيمة الفاتورة"
                            : "Discount on delivery fee based on invoice total"}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-amber-50/50 border border-amber-100 p-5 rounded-[25px] flex gap-4 items-center shadow-sm">
                ...
                <p className="text-[12px] text-amber-900 font-[700] leading-relaxed m-0">
                    {isAr
                        ? "يُطبَّق الخصم على سعر التوصيل فقط بناءً على مجموع الأصناف — مستقل عن خصم الفرع."
                        : "Discount applies to delivery fee only based on items subtotal — independent of branch discount."}
                </p>
            </div>
